...
class Solution(object):
    def nodesBetweenCriticalPoints(self, head):
        ...
        :rtype: List[int]
        """
        # BSB / SBS / not the ends
        # the pattern must always be max -> min (or vice versa) 

        # there must be enough elemets for the pattern to exist
        ...
        if len(critical_indices) < 2: # not enough minima/maxima to calculate distances
            return [-1, -1]
        ...
        # max distance is always that between the 2 outmost critical points
