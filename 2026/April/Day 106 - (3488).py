...
class Solution(object):
    def solveQueries(self, nums, queries):
        ...
        ans = []
        # Binary Search for adjacent value for each querie

        for q_idx in queries:
            val = nums[q_idx]
            indices = pos_dict[val]
            
            # no 'other' indicie exists
            if len(indices) < 2:
                ans.append(-1)
                continue
            
            # since q_idx is guaranteed to be in indices, bisect_left gives its exact position
            ...
            # circular Distances
            # Distance to left
            d_left = abs(q_idx - left_neighbor)
            d_left = min(d_left, n - d_left)
            
            # Distance to right
            d_right = abs(q_idx - right_neighbor)
