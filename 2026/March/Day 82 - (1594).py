...
class Solution(object):
    def maxProductPath(self, grid):
        ...
        # first row - only possible from right moves
        for j in range(1, n):
            max_dp[0][j] = min_dp[0][j] = max_dp[0][j-1] * grid[0][j]
            
        # first column - only possible from down moves
        for i in range(1, m):
            max_dp[i][0] = min_dp[i][0] = max_dp[i-1][0] * grid[i][0]
            
        # complete the grid - stop at the end point (following points can never result in a valid move to get to this point - once here, that is it)
        for i in range(1, m):
            for j in range(1, n):
                val = grid[i][j]
                
                # possible values from each previous location
                options = (
                    max_dp[i-1][j] * val,
                    min_dp[i-1][j] * val,
                    max_dp[i][j-1] * val,
