...
# binary. For example, 21 written in binary is 10101, which has 3 set bits.
...
class Solution(object):
    def countPrimeSetBits(self, left, right):
        ...
        :rtype: int
        """
        #helper function to determine if it is a prime number
        def is_prime(n):
            if n < 2:
                return False
        ...
        count = 0
        #for everthing in the range
        for i in range (left, right+1):
            #convert to binary
            #count the number of active bits
            active = bin(i).count('1')
            #check if this count is prime?
            if is_prime(active):
                count += 1
