...
# Given an integer turnedOn which represents the number of LEDs that are currently on (ignoring the PM), return all possible
...
class Solution(object):
    def readBinaryWatch(self, turnedOn):
        ...
        """
        result = []
        #test all valid hours (0-11) and minutes (0-59) and see if it is a valid tiem that can be made
        for hour in range(12):
            for minute in range(60):
                #howmany 1s in this time
                # bin(n).count('1') counts set bits
